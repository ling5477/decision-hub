...
## Current authority — Stage-QDR-7 capacity harness runtime blocker-3 closed locally
...
历史formal runs（包括Retry-3 run `20260716T144346Z`）继续保留为`BLOCKED / CAPACITY_HARNESS_RUNTIME_DEFECT`。Blocker-3已在implementation-validation run `20260717T122621Z`中关闭tenant、Context restart与nonce driver问题；该run保持0/15且`capacityAcceptanceExecuted=false`，不构成formal capacity PASS。下一任务仅允许`DH-STAGE-QDR-7-B2-CAPACITY-HARNESS-CI-VERIFICATION`；远端CI green前不得执行Retry-4或进入B3。
