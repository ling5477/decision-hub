 ...
 * <p>仅用于在单进程测试内验证防重放逻辑。
 *
 * <p>注意（DH P1-4 residual）：本实现只是单实例内存，<b>不</b>满足生产/多实例防重放要求。 进入 Integration-1 前必须实现持久化或集中缓存的 nonce store（replay
 * nonce persistence）， 该残留不阻塞本轮 contract test，但阻塞 Integration-1。
 */
