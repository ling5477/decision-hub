...
/**
 * Int0ContractValidator 是 test-only DH-NQ Integration-0 契约校验器。
 *
 * <p>它把已冻结的安全策略组合成一个校验管线：header 存在性 -> content-type -> source allowlist -> payload 上限 -> timestamp 窗口 -> HMAC
 * 签名 -> nonce 防重放 -> tenant 绑定 -> 禁止字段 -> 禁止能力 -> schema。任一步失败按冻结拒绝码返回，并产出脱敏 audit event。
 *
 * <p>本类不连接任何真实 NQ、真实 DH client（RealClient）、真实 Provider、真实交易所或真实凭证；纯内存校验。
 */
public final class Int0ContractValidator {
  ...
  public Int0ValidationResult validate(
    ...
    if (size > Int0Contract.MAX_PAYLOAD_BYTES) {
      return Int0ValidationResult.reject(
          ...
          Int0AuditEvent.rejected(
              Int0AuditEvent.PAYLOAD_TOO_LARGE,
              source,
              tenant,
              requestId,
              traceId,
    ...
    String expected =
        Int0Signing.hmacSha256Hex(Int0Signing.canonical(headers, body), secret);
    if (!Int0Signing.constantTimeEquals(expected, headers.get(Int0Contract.H_SIGNATURE))) {
      return Int0ValidationResult.reject(
          ...
          Int0AuditEvent.rejected(
              Int0AuditEvent.SIGNATURE_FAILED,
              source,
              tenant,
              requestId,
              traceId,
    ...
    // nonce 防重放（409）
    boolean firstSeen =
        nonceStore.registerIfAbsent(source, tenant, requestId, headers.get(Int0Contract.H_NONCE));
    if (!firstSeen) {
      return Int0ValidationResult.reject(
          409,
          "REPLAY",
    ...
    if (!forbiddenFields.isEmpty()) {
      return Int0ValidationResult.reject(
          ...
          forbiddenFields,
          Int0AuditEvent.rejected(
              Int0AuditEvent.FORBIDDEN_FIELD_REJECTED,
              source,
              tenant,
              requestId,
              traceId,
    ...
    return Int0ValidationResult.accept(
        Int0AuditEvent.received(source, tenant, requestId, traceId));
  }
  ...
  private void scanFieldNames(
      JsonNode node, String path, List<String> forbidden, List<String> hits) {
    if (node == null) {
      return;
    }
    if (node.isObject()) {
