...
try {
    ...
    $report = [pscustomobject]@{
        schema_version = "3.0.0"; status = $status; checker_version = $checkerVersion; ruleset_version = $rulesetVersion; huangshan_source_ref = "6c59c8c36ecd8722c712d5685b8c3822c1c8b030"; previous_baseline_head = $previousBaseline.previous_baseline_head; current_baseline_head = $previousBaseline.current_baseline_head; java_platform = "release-$($platform.java.compiler_release)"; spring_platform = "boot-$($platform.spring.boot)_framework-$($platform.spring.framework)"; configuration_hash_algorithm = $configurationHashAlgorithm; configuration_sha256 = $configurationHash; previous_count = [int]$previousBaseline.previous_count; current_count = $reported.Count; persisted_count = [int]$previousBaseline.persisted_count; resolved_count = [int]$previousBaseline.resolved_count; previous_baseline_finding_count = $previousCount; ruleset_expansion_count = $expansionCount; upstream_sync_count = $upstreamCount; current_task_count = $currentTaskCount; report_artifact = $OutputPath.Replace("\", "/"); violations = $reported
    }
