...
try {
    ...
    $songshanMappingPath = Join-Path $standardRoot "history\alibaba-songshan-rule-mapping.yaml"
    Assert-Condition (Test-Path -LiteralPath $songshanMappingPath -PathType Leaf) "MAPPING_INVALID" "Songshan history mapping missing"
    $songshanMapping = Get-Content -LiteralPath $songshanMappingPath -Raw -Encoding UTF8
    ...
    Assert-Condition ($baseline.schema_version -eq '3.0.0' -and $baseline.current_ruleset_version -eq 'huangshan-platform-2.0.0') "BASELINE_SCHEMA_INVALID" "baseline schema/ruleset invalid"
    ...
    Assert-Condition ($baseline.previous_baseline_head -match '^[0-9a-f]{40}$' -and $baseline.current_baseline_head -match '^[0-9a-f]{40}$') "BASELINE_SCHEMA_INVALID" "baseline HEAD identities invalid"
    Assert-Condition ([int]$baseline.persisted_count + [int]$baseline.resolved_count -eq [int]$baseline.previous_count) "BASELINE_SCHEMA_INVALID" "persisted/resolved counts do not reconcile"
    Assert-Condition ([int]$baseline.previous_baseline_finding_count + [int]$baseline.ruleset_expansion_count + [int]$baseline.upstream_sync_count + [int]$baseline.current_task_count -eq [int]$baseline.current_count) "BASELINE_SCHEMA_INVALID" "baseline classifications do not sum"
    Assert-Condition ([int]$baseline.current_task_count -eq 0) "BASELINE_SCHEMA_INVALID" "current governance task must not create Java findings"
    ...
    $governed = @(); $governed += @(Get-ChildItem -LiteralPath $standardRoot -Recurse -File); $governed += @(Get-Item $skillPath); $governed += @(Get-ChildItem -LiteralPath $PSScriptRoot -File)
    ...
    Write-Output "PREVIOUS_BASELINE_HEAD=$($baseline.previous_baseline_head)"
    Write-Output "CURRENT_BASELINE_HEAD=$($baseline.current_baseline_head)"
    Write-Output "BASELINE_PREVIOUS_COUNT=$($baseline.previous_count)"
    Write-Output "BASELINE_CURRENT_COUNT=$($baseline.current_count)"
    Write-Output "BASELINE_PERSISTED=$($baseline.persisted_count)"
    Write-Output "BASELINE_RESOLVED=$($baseline.resolved_count)"
    Write-Output "BASELINE_PREVIOUS_FINDINGS=$($baseline.previous_baseline_finding_count)"
    Write-Output "BASELINE_RULESET_EXPANSION=$($baseline.ruleset_expansion_count)"
    Write-Output "BASELINE_UPSTREAM_SYNC=$($baseline.upstream_sync_count)"
    Write-Output "BASELINE_CURRENT_TASK=$($baseline.current_task_count)"
