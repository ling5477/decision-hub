...
### 4.1 P4 gate-fix contract review conclusion（2026-07-03）
...
| 项 | P4 分类 | 结论 |
| --- | --- | --- |
| `dryRun` | `DOC_ONLY_ALIAS` | 当前 request/output schema 不含该字段；后续 WO 可把 dry-run 语义写成 `source=NQ_DRYRUN` planning alias 与 no-side-effect assertion，不得要求 wire-level `dryRun`。 |
