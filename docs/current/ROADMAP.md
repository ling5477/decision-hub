...
## Current route — 2026-07-18 harness stabilization closeout accepted
...
Hard-ceiling authority、command bypass与calibration path blocker保持关闭，历史0轮matrix、threshold retry-2、无效same-pool probe与formal轨迹不变。Criteria保持`FROZEN / ACCEPTED`，B2不回退。harness stabilization已由15/15非正式qualification关闭；下一步只允许获得push授权并执行新commit的exact-SHA CI，CI green后恢复原Retry-4，不创建Retry-5或进入B3。
