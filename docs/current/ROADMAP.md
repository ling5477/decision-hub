...
当前路线：Plan 与 Work Order 已本地完成；先发布两提交并运行 exact-SHA CI，再按 B1 schema/contracts → milestone review → B2 PostgreSQL/JDBC persistence → milestone review → B3 internal historical read model → B4 integrity/retention safety review → B5 final close 推进。API 与 automatic learning 不在 Stage-QDR-9 路线内。
