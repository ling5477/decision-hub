...
>
> 当前 API 文档只记录已实现 API 与明确的未实现 API 边界。它不是 Stage-QDR-4 tag close 或 docs discipline cleanup 的默认 blocker source；只有出现 `FACTSOURCE_POLICY.md` 定义的硬错误时才可升级为 blocker。

## 1. 当前状态

```text
当前任务: DH-DOCS-DISCIPLINE-CLEANUP-IMPLEMENTATION / NO_API_CHANGE
当前阶段: STAGE_QDR_4 / CLOSED / ACCEPTED / ARCHIVED / NO_API_CHANGE
当前 tag: STAGE_QDR_4_TAG / PENDING
下一步: DH-STAGE-QDR-4-TAG-CLOSE / TAG_CLOSE_ONLY / NO_API_CHANGE
```
...
B3 Persistence Baseline: CLOSED / ACCEPTED / COMMITTED / V8_JDBC_BASELINE / NO_ENDPOINT
B4 QDR Pipeline Integration: DONE / FREEZE ACCEPTED / COMMITTED / EXISTING_DRY_RUN_PIPELINE_ONLY / NO_ENDPOINT
B5 close review: YES / ACCEPTED / NO_ENDPOINT
stage-qdr-3 final close: CLOSED / ACCEPTED / NO_API_CHANGE
stage-qdr-4: CLOSED / ACCEPTED / ARCHIVED / NO_API_CHANGE
...
OpenAPI 仍为正式契约单源；Stage-QDR-4 已 `CLOSED / ACCEPTED / ARCHIVED`，tag 仍为 `PENDING`，本轮 docs discipline cleanup 不修改 API、Controller、OpenAPI、JSON Schema、contracts 或 golden_cases。DH Stage4 Decision Pipeline MVP K1-K8 已 `CLOSED / ACCEPTED`；`DecisionRequest` / `DecisionOutput` 已作为 K1 domain contract 与 JSON Schema 落地；audit / snapshot / trace persistence 与 internal replay read model 已在 usecase/infra 内闭环，但 replay execution API 仍未实现。`NQ-DH-I1-DH-LIMITED-RUNTIME-ENDPOINT-IMPLEMENTATION` 已在 DH 侧实现受限 inbound endpoint `POST /api/ai/decision-dry-runs`，该 endpoint 默认关闭，仅 dev/test profile 可显式启用，production profile disabled / kill switch fail-closed。`stage-qdr-1` 已关闭：成功 dry-run 会创建 `decision_request`、`decision_run`、`quant_signal` 与 `quant_decision`，并继续保留 V5 `dh_decision_*` audit / trace / output 链路。stage-qdr-2 B1/B2/B3/B4 已 `CLOSED / ACCEPTED / COMMITTED`；B4 已新增 tenant-bound approval create / get / decision API 与审计事件写入。approval API 只改变 DH 内部 approval 状态并写审计，不触发 NQ、交易、provider、HTTP 或 replay execution。stage-qdr-3 planning 与 implementation work order 已完成；B1 已 `DONE / COMMITTED`；B2/B3/B4 已 `DONE / FREEZE ACCEPTED / COMMITTED`；B5 close review 已 `YES / ACCEPTED`，stage-qdr-3 final close 已 `CLOSED / ACCEPTED`。B4 只更新既有 `POST /api/ai/decision-dry-runs` 的内部 dry-run / QDR pipeline：成功 response 的 trace summary 可包含 redacted model gateway refs，不新增 endpoint、Controller、OpenAPI、Provider API、Model Gateway API 或 Prompt Version API。任何 future Model Gateway / Provider / Prompt Version API 必须另起 API / contract / security review。`NQ_DRYRUN` 只进入 dev/test allowlist，不进入 production allowlist；实现不包含 NQ runtime client implementation、真实 outbound HTTP、real provider、Agent / LangGraph runtime 或 LIVE。

### 1.1 stage-qdr-3 B4 API 结论

```text
B4 new endpoint: NO
B4 new Controller: NO
B4 OpenAPI change: NO
B4 Model Gateway API: NOT STARTED
B4 Provider API: NOT STARTED
B4 Prompt Version API: NOT STARTED
B4 replay execution API: NOT STARTED
B5: YES / ACCEPTED / NO_API_CHANGE
stage-qdr-3 final close: CLOSED / ACCEPTED / NO_API_CHANGE
stage-qdr-4: CLOSED / ACCEPTED / ARCHIVED / NO_API_CHANGE
stage-qdr-4 tag: PENDING
```

B4 pipeline integration 只服务既有 dry-run endpoint 的内部 mock gateway trace/ref enrichment，不改变 wire contract。成功 response 仍是 read-only snapshot；trace summary 可包含 `modelGateway:MODEL_GATEWAY_MOCK_CALL`、`promptVersion`、`modelVersion`、`providerProfile`、`gatewayCall`、`trust`、`redaction`、`budget` 等 redacted refs。response 不暴露 raw prompt、raw provider response、credential、token、cookie、apiKey、apiSecret 或 passphrase，不包含 BUY / SELL / PLACE_ORDER / CANCEL_ORDER execution action。B4 不授权真实 provider、真实 HTTP、Provider SDK、LangGraph / AutoGen / CrewAI、Agent runtime、NQ mutation 或 LIVE。
