...
## 2026-07-12 Stage-QDR-7 B1 source normalization blocker fix
...
`DecisionDryRunRuntimeProperties` 已删除 request source lowercase；runtime allowlist与tenant/source pair均按canonical `NQ_DRYRUN` exact match。配置条目只trim外层空白，noncanonical/blank/trailing-empty/矛盾pair均fail-closed。HMAC canonical material、Controller/DTO/OpenAPI、nonce/replay、rate/idempotency和runtime boundary未变。全仓Maven测试与Docker/Testcontainers PostgreSQL regression通过；capacity benchmark仍未运行。
