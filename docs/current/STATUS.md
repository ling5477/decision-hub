...
model gateway: IMPLEMENTED_BY_VALIDATION / MOCK_ONLY / USECASE_ONLY / NO_API.
model gateway persistence: IMPLEMENTED_BY_VALIDATION / V8 / JDBC_BASELINE / NO_API.
...
Task type: CODE_CHANGE + TEST + MINIMAL_DOCUMENTATION + STAGE_QDR_3_B4 + QDR_PIPELINE_INTEGRATION + MOCK_MODEL_GATEWAY_INTEGRATION + MODEL_GATEWAY_AUDIT_TRACE + FAIL_CLOSED_DECISION_PIPELINE + NO_DB_MIGRATION + NO_API_CHANGE + NO_REAL_PROVIDER + NO_REAL_HTTP + NO_AGENT + NO_LANGGRAPH + NO_LIVE
...
B4 只把既有 QDR decision / dry-run pipeline 接入 B1/B2/B3 的 mock `ModelGatewayPort`、prompt/model/provider registry、gateway call persistence 与 V5 audit/trace metadata。pipeline 调用经过 `QdrModelGatewayIntegrationPort` -> `ModelGatewayPort` / `ModelGatewayService`，强制 `ProviderTrustPolicy`、prompt/model version registry lookup、budget、redaction 与 prompt injection guard。

成功路径只把 redacted gateway result 作为 reasoning / evidence summary，并把 `promptVersionId`、`modelVersionId`、`providerProfileId`、`gatewayCallRef`、`trustDecision`、`redactionStatus`、`budgetSummary` 写入 decision trace summary / audit payload / `quant_decision.constraints_json` safe refs。gateway result 不改变 approval status，不触发 replay execution、NQ、交易、订单、撤单、账户、ledger、risk、paper 或 live mutation。

B4 未新增 API / Controller / OpenAPI，未新增 migration，未修改 V1-V8 migration，未接真实 provider、真实 HTTP、Provider SDK、Agent / LangGraph、NQ 或 LIVE。gateway failure、provider policy denied、budget exceeded、prompt injection denied、redaction failure、registry mismatch、gateway call persistence failure、audit failure、trace failure 与 unknown exception 均 fail-closed；不保存 raw prompt，不保存 raw provider response，不保存 credential。
...
## 1.0.42 DH-STAGE-QDR-3 B3 Blocker Fix（2026-07-07，DONE）

```text
Task: DH-STAGE-QDR-3-B3-BLOCKER-FIX
Task type: BLOCKER_FIX + DOCUMENTATION_FIX + TOOLING_VALIDATION_RECOVERY + STAGE_QDR_3_B3_FREEZE_BLOCKER_FIX + NO_BUSINESS_CODE_CHANGE + NO_DB_MIGRATION_CHANGE + NO_API_CHANGE + NO_REAL_PROVIDER + NO_REAL_HTTP + NO_AGENT + NO_LANGGRAPH + NO_LIVE
stage-qdr-3 B1: COMMITTED
stage-qdr-3 B2: CLOSED / ACCEPTED / COMMITTED
...
本轮只修复 B3 review/freeze blocker：`docs/current/DB_SCHEMA.md` 的 V8 schema 记录已按实际 `V8__qdr_model_gateway_persistence_baseline.sql` 对齐，去除 planned / stale 字段名，补齐实际字段、unique/index、check constraint 与 raw storage prohibition 说明。未修改 V1-V8 migration、Java 生产代码、测试代码、API、Controller、JDBC repository 或 NQ 仓库。

Maven validation 已恢复并通过：`mvn -ntp -pl dh-domain -am test`、`mvn -ntp -pl dh-usecase -am test`、`mvn -ntp -pl dh-infra -am test`、`mvn -ntp -pl dh-app -am test`、`mvn -ntp -Pquality validate` 均为 `BUILD SUCCESS`；未使用 `-DskipTests` 或 `-DskipITs`。本轮未执行 Maven local repository cleanup，未使用临时 `maven.repo.local`。
...
## 1.0.41 DH-STAGE-QDR-3 B3 Persistence Baseline（2026-07-07，IMPLEMENTED_BY_VALIDATION）

```text
Task: DH-STAGE-QDR-3-B3-PERSISTENCE-BASELINE
Task type: CODE_CHANGE + TEST + DB_MIGRATION + MINIMAL_DOCUMENTATION + STAGE_QDR_3_B3 + PROMPT_MODEL_VERSION_PERSISTENCE + MODEL_GATEWAY_CALL_PERSISTENCE + JDBC_REPOSITORY + AUDIT_REDACTION_STORAGE_BOUNDARY + NO_API_CHANGE + NO_REAL_PROVIDER + NO_REAL_HTTP + NO_AGENT + NO_LANGGRAPH + NO_LIVE
stage-qdr-3 B1: COMMITTED
stage-qdr-3 B2: CLOSED / ACCEPTED / COMMITTED
...
B3 新增 `V8__qdr_model_gateway_persistence_baseline.sql`、tenant-bound prompt/model version persistence ports、JDBC repositories、migration presence tests、JDBC fail-closed tests 与 ArchitectureTest guard。V8 只保存 `qdr_prompt_template`、`qdr_prompt_version`、`qdr_model_profile`、`qdr_model_version`、`qdr_model_gateway_call` 的 safe metadata / hash / ref / redacted summary；不保存 raw prompt，不保存 raw provider response，不保存 credential / apiKey / apiSecret / token / passphrase。B3 未新增 API / Controller / OpenAPI，未接真实 provider、真实 HTTP、Provider SDK、Agent / LangGraph、NQ 或 LIVE。
...
Task type: CODE_CHANGE + TEST + MINIMAL_DOCUMENTATION + STAGE_QDR_3_B1 + NO_DB_MIGRATION + NO_API_CHANGE + NO_REAL_PROVIDER + NO_REAL_HTTP + NO_AGENT + NO_LANGGRAPH + NO_LIVE
stage-qdr-3 B1: IMPLEMENTED_BY_VALIDATION
B2: NOT STARTED
B3: NOT STARTED
B4: NOT STARTED
real HTTP: NO
...
B1 只新增 QDR prompt/model version domain baseline、tenant-bound in-memory/mock registry、`PromptRenderPolicy` contract、deterministic `PromptInjectionGuard` contract、checksum validation 与 architecture guard。实现不新增 API / Controller / OpenAPI，不新增 migration，不修改历史 migration，不接真实 provider，不接真实 HTTP，不启动 Agent / LangGraph，不触碰 NQ、交易、订单、账户、ledger、risk、paper 或 LIVE。
...
```text
Task: DH-STAGE-QDR-2-B3-HUMAN-APPROVAL-MIGRATION-AND-DOMAIN
Task type: CODE_CHANGE + TEST + DOCUMENTATION + DB_MIGRATION + HUMAN_APPROVAL_DOMAIN + HUMAN_APPROVAL_REPOSITORY + STATE_MACHINE + SECURITY_BOUNDARY_PRESERVATION + NO_API_IMPLEMENTATION + NO_APPROVAL_API + NO_REAL_HTTP + NO_PROVIDER + NO_AGENT
stage-qdr-2 WO: DONE / WORK_ORDER_PARTIALLY_CONSUMED
stage-qdr-2 B1: DONE / IMPLEMENTED_BY_VALIDATION
stage-qdr-2 B2: CLOSED / ACCEPTED
stage-qdr-2 B3: DONE / IMPLEMENTED
stage-qdr-2 implementation overall: PARTIAL
human_approval_packet: MIGRATION_ADDED / V7__human_approval_packet.sql
Approval domain/status machine: IMPLEMENTED
Approval repository port/service: IMPLEMENTED / INTERNAL_ONLY
JDBC repository adapter: IMPLEMENTED / TENANT_BOUND
...
B3 新增 `human_approval_packet` Flyway migration、approval domain model、状态机、repository port / service、JDBC repository adapter、migration presence test、domain/status transition tests、repository tests 与 architecture guard。`human_approval_packet` 只记录 DH 内部人工审查证据，不是交易授权；`APPROVED` 不等于 `BUY`，`REJECTED` 不等于 `SELL`，`LONG_BIAS / SHORT_BIAS` 不映射为 `BUY / SELL`。

B3 未新增 Controller、approval API、approval decision endpoint、WebMvc approval tests、replay execution API、真实 HTTP outbound、real provider、OpenAI / Anthropic / Gemini / Ollama SDK、LangGraph / AutoGen / CrewAI、NQ mutation、交易链路或 LIVE。B4 才允许在单独授权下实现 approval API / write endpoint；B5 仍 `NOT STARTED`。
