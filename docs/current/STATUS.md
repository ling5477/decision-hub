...
MIGRATION_HISTORY_GATE: IMPLEMENTED
CONSTRAINT_FINGERPRINT: IMPLEMENTED
BOUNDED_LEGACY_REPAIR: IMPLEMENTED / CEILING_1000
...
现有`beforeMigrate`已删除，`beforeEachMigrate`与V13在同一PostgreSQL事务运行。targeted PostgreSQL 17/Testcontainers已覆盖rollback、retry、timeout、bounded repair、completed no-op和V14 no-truncation；历史milestone review和retry-2的`BLOCKED`结论保持不变，B2不得提前accepted。
