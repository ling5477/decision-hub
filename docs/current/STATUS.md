...
## 2026-07-13 Stage-QDR-7 B2 schema errata implementation review

```text
STAGE_QDR_7_B1: FROZEN
STAGE_QDR_7_B2_SCHEMA_ERRATA_IMPLEMENTATION_REVIEW: BLOCKED
STAGE_QDR_7_B2_SCHEMA_ERRATA_IMPLEMENTATION: BLOCKED / P1_FIX_REQUIRED
B2_MILESTONE_ACCEPTANCE: NOT_YET
BEFORE_EACH_MIGRATE_CALLBACK: REJECTED / LOCK_TIMEOUT_SCOPE_GAP
MIGRATION_HISTORY_GATE: PASS
CONSTRAINT_FINGERPRINT: PASS / PRECHECK_LOCK_UNBOUNDED
BOUNDED_LEGACY_REPAIR: PASS / CEILING_1000
LOCK_TIMEOUT_CONTRACT: FAIL / SET_AFTER_PRECHECK
STATEMENT_TIMEOUT_CONTRACT: PASS / TRANSACTION_LOCAL
CURRENT_FACTSOURCE_CONSISTENCY: FAIL / OLD_ENTRY_RESIDUAL
...
独立PostgreSQL 17.10实验确认callback在设置`SET LOCAL lock_timeout = '5s'`前执行constraint/data precheck；`ACCESS EXCLUSIVE`锁使precheck等待超过5秒，最终由实验外加的8秒statement timeout终止。既有11项suite与full 1075 tests全绿但未覆盖该分支。root/current README与CODEX instruction另有旧B2状态残留且不在本轮allowlist。B2不得accepted。
