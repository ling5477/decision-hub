...
## 2026-07-12 Stage-QDR-7 B2 schema errata review

```text
STAGE_QDR_7_B1: FROZEN
STAGE_QDR_7_B2: BLOCKED / SCHEMA_ERRATA_REVIEWED
STAGE_QDR_7_B2_SCHEMA_ERRATA_REVIEW: PASS / TRANSACTIONAL_CALLBACK_SELECTED
...
现有`beforeMigrate`无法与失败V13共同rollback；唯一获准路线为`beforeEachMigrate` transactional callback。历史milestone review和retry-2的`BLOCKED`结论保持不变。
