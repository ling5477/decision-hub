...
以下数据流只保留为历史背景，不代表当前系统已经开始集成，也不是当前 implementation。当前唯一下一步是 `NQ-DH-I1-M3-JOINT-MOCK-FIXTURES-AND-CONTRACT-TESTS-WO / NOT STARTED / WORK_ORDER_ONLY_ALLOWED`，且仍为 work order / mock contract planning，不是 runtime implementation。
