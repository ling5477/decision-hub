...
B3 planning 与 implementation work order scope contracts 继续冻结；入口为 [B3 plan](DH_STAGE_QDR_7_B3_PLAN.md) 与 [B3 implementation work order](DH_STAGE_QDR_7_B3_IMPLEMENTATION_WORK_ORDER.md)。B3.1-B3.3 已通过本地 full regression、PostgreSQL/Testcontainers 与 quality 验收；B2 capacity gate 保持 deferred。B3 仍不开放生产、真实 HTTP、Provider、NQ、Agent、LangGraph、Paper 或 LIVE，最终关闭等待 exact-SHA 远端 CI。
