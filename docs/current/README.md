...
`dh-security -> dh-domain` 已由已实现的单向依赖使用，且只导入 canonical `FeedbackEnvironment`。persistent guard PostgreSQL fixture 已用显式 signed DEV environment 与 verified scope 升级，并已通过 Testcontainers、完整回归和质量门；effective upstream scope 为 `48 / 48 PASS`，upstream contract 为 `IMPLEMENTED / LOCAL_ACCEPTED`。
