...
B1、B2 与 B3 均已发布并接受；B3 historical evidence read model 为 internal-only、read-only。retention 仍为 `NOT_STARTED`；B4 仅可在本 authority commit 的 exact-SHA CI 通过后作为独立下一任务启动，API 与 automatic learning 均不允许。
