...
Stage-QDR-4、Stage-QDR-5 与 Stage-QDR-6 均已 `CLOSED / ACCEPTED / ARCHIVED / TAGGED`。Stage-QDR-7 B1为`FROZEN`，B2为`BLOCKER_FIX_RETRY_DONE / REVIEW_PENDING`；下一步仅允许`DH-STAGE-QDR-7-B2-PERSISTENT-GUARDS-MILESTONE-REVIEW-RETRY-2`。capacity acceptance与B3仍为`NOT_ALLOWED`，不授权API或runtime扩张。
