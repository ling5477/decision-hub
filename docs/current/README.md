...
`dh-security -> dh-domain` 已经 DAG 审计批准，但当前尚未修改 POM；唯一 future POM 范围为 `dh-security/pom.xml`，只为引用 canonical `FeedbackEnvironment`。有效 upstream scope 为 `47 / 47 PASS`，upstream implementation 仍在任何代码提交前阻断。
