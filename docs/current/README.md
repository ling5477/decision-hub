...
> 事实源: docs/current
>
> NQ / DH 三轮只读审计已完成；Integration-0 safety gate CLOSED / ACCEPTED；DH not integrated；runtime integration NOT STARTED；Integration-1 runtime implementation NOT STARTED；IMP3 joint mock contract tests 已以 test-support / mock-only 方式实现；no RealClient；no real provider；no trading ability；LIVE DISABLED。
