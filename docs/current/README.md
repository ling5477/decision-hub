...
```text
DH-GATEK-DECISION-PIPELINE-MVP-K7-GOLDEN-CASES-EVAL（NOT STARTED）。
...
执行口径：
  ...
  - K5 Provider Health / Budget / Latency 已实现 mock-only health / budget / latency guard；K6 Mock NQ Dry-run Contract Tests 已实现 mock-only dry-run contract tests；K7 仍 NOT STARTED，不得连续进入 K8。
