...
## Current disposition（2026-07-18）

Capacity acceptance criteria保持`FROZEN / ACCEPTED`，B2保持`CLOSED / ACCEPTED`。历史Retry-4 formal run `20260718T065630Z`仍为`BLOCKED / CAPACITY_HARNESS_RUNTIME_DEFECT`；本轮没有执行formal acceptance，也没有改写任何历史formal verdict。qualification run `20260718T130056Z`以正式fixture、threshold comparator、full regression、quality、artifact/secret/teardown合同完整通过15/15 mandatory scenarios，harness stabilization因此`CLOSED / ACCEPTED`。当前结论为：
...
LATEST_FORMAL_RUN_ID: 20260718T065630Z
LATEST_FORMAL_RESULT: BLOCKED / CAPACITY_HARNESS_RUNTIME_DEFECT / INTERNAL EXIT 20 / SUMMARY 0 OF 15
LATEST_FORMAL_CAPACITY_ACCEPTANCE_EXECUTED: false
QUALIFICATION_RUN_ID: 20260718T130056Z
...
QUALIFICATION_FULL_REGRESSION: PASS / 19 OF 19 REACTOR SUCCESS / 1144 / 0 FAILURES / 0 ERRORS / 0 SKIPPED
...
Qualification修复并验证了合法decision-output FK fixture、scenario独立setup与继续执行、真实execution-state/verdict ledger、partial threshold comparison、finalizer和三类restart真实计数；Windows Maven非UTF-8本地日志改用ASCII-compatible读取，qualification profile仅跳过外层`dh-app` Surefire重复回归，默认`mvn test`不受影响。Qualification不是capacity acceptance，不能覆盖上方historical formal blockers；远端exact-SHA CI通过前不得恢复Retry-4。
