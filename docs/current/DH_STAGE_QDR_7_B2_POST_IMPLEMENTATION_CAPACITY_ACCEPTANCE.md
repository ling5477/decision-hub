...
Capacity acceptance criteria保持`FROZEN / ACCEPTED`，B2保持`CLOSED / ACCEPTED`。formal run `20260715T140521Z`作为`BLOCKED / CAPACITY_HARNESS_RUNTIME_DEFECT`历史结果保留。后续runtime binding blocker已关闭；binding validation `20260715T145836Z`真实进入PowerShell preflight，并按dirty worktree与可用内存不足16 GiB返回internal exit `10`和完整blocked artifact。该run不是formal acceptance retry，因此当前验收结论仍为：

```text
POST_B2_CAPACITY_ACCEPTANCE: BLOCKED / FORMAL RETRY REQUIRED ON QUALIFIED ENVIRONMENT
...
FORMAL_RUN_ID: 20260715T140521Z
MAVEN_EXIT_CODE: 1
HISTORICAL_INTERNAL_EXIT_CODE: NOT_AVAILABLE
HARNESS_RUNTIME_BINDING: CLOSED / ACCEPTED
LATEST_BINDING_VALIDATION_RUN_ID: 20260715T145836Z
LATEST_BINDING_VALIDATION: BLOCKED / ENVIRONMENT_CAPACITY_PREFLIGHT_BLOCKED / INTERNAL EXIT 10
LATEST_BINDING_VALIDATION_ARTIFACT: PASS / 26 FILES / 0 MANIFEST MISMATCH / 0 SECRET FINDINGS
B2_IMPLEMENTATION_STATUS: CLOSED / ACCEPTED
Stage-QDR-7 B3: NOT_ALLOWED
next task: DH-STAGE-QDR-7-B2-POST-IMPLEMENTATION-CAPACITY-ACCEPTANCE-RETRY-2
```

该`BLOCKED`不是capacity FAIL，也不回退B2 acceptance；本任务普通回归`1134 / 0 / 0 / 0`和binding artifact不能替代未执行的formal scenarios。历史formal记录见`DH_STAGE_QDR_7_B2_FORMAL_CAPACITY_ACCEPTANCE_RESULT.md`。
