...
当前已关闭 planning task；下一工单必须是 `DH-STAGE-QDR-9-IMPLEMENTATION-WORK-ORDER`。该工单只允许枚举 B1-B4 exact files、独立 migration review 输入和 6/6 scope，不允许同步开始 implementation。
