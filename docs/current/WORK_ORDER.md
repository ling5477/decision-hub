...
```text
current task: DH-STAGE-QDR-6-FINAL-CLOSE-REVIEW
current task status: BLOCKED / CURRENT_FACTSOURCE_CONFLICT / REVIEW_ONLY
next action: DH-STAGE-QDR-6-FINAL-CLOSE-BLOCKER-FIX
mode: CURRENT_FACTSOURCE_SYNC_ONLY + NO_ARCHIVE + NO_TAG + NO_CODE + NO_SCHEMA + NO_API + NO_PROVIDER + NO_AGENT + NO_LIVE
```

Stage-QDR-6 B1/B2、B3 `CLOSED / ACCEPTED` 与 B4 internal report 均已完成；final close 因 `README.md`、`docs/current/README.md`、`docs/current/CODEX_PROJECT_INSTRUCTIONS.md` 的 current factsource 冲突而 `BLOCKED`。下一步只允许同步这些 current facts 并重跑 final close review；不得创建 archive packet/tag，不得进入 Stage-QDR-7 planning，不得扩展 schema、port/JDBC、API、Provider、NQ、Agent、LangGraph、交易或 LIVE。
...
ALLOW_STAGE_QDR_6_FINAL_CLOSE_REVIEW: YES / CONSUMED / BLOCKED
...
ALLOW_STAGE_QDR_6_B4_IMPLEMENTATION_NOW: YES / NEXT_TASK_ONLY / INTERNAL_REPORT_ONLY
