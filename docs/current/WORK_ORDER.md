...
mode: ARCHIVE_BEFORE_TAG_NEXT_ONLY + NO_STAGE_QDR_7_PLAN + NO_CODE + NO_TEST + NO_SCHEMA + NO_API + NO_PROVIDER + NO_AGENT + NO_LIVE
```

Stage-QDR-6 final close retry 已 `PASS`；previous final close review 的 `BLOCKED` 历史结论保持不变。下一步只允许先形成并提交 self-contained archive packet，再在 clean worktree 上执行 tag close；不得在本任务创建 archive/tag，不得进入 Stage-QDR-7 planning，不得扩展 schema、port/JDBC、API、Provider、NQ、Agent、LangGraph、交易或 LIVE。
