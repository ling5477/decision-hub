...
上一轮threshold evidence retry仍是历史失败证据：首个protected请求为`200`，同一Context第二个合法请求返回`500 / UNKNOWN_ERROR`，rate matrix为0轮，cleanup与PostgreSQL contention mandatory evidence不完整。本轮确定性baseline profile已使同一Context与packaged jar各自的5次顺序、8次并发请求全部返回结构化2xx，完整1101项回归及质量门通过；该结果只关闭calibration path blocker。下一任务仅允许`DH-STAGE-QDR-7-B2-CAPACITY-THRESHOLD-EVIDENCE-RETRY-2`重新采集阈值证据，不授权criteria freeze、formal harness、capacity acceptance或B3；B2 acceptance不回退。
