...
B2保持`CLOSED / ACCEPTED`，criteria保持`FROZEN / ACCEPTED`，全部formal blocked run继续作为历史证据保留。Qualification run `20260718T130056Z`以`NOT_FORMAL / QUALIFICATION_ONLY`完成15/15 scenarios与94/94 comparisons，只关闭harness stabilization，不产生capacity PASS。当前只允许获得push授权并执行新commit的exact-SHA CI；CI green后恢复原Retry-4，Post-B2 capacity acceptance继续`BLOCKED / FINAL FORMAL ACCEPTANCE PENDING`，B3继续`NOT_ALLOWED`。
