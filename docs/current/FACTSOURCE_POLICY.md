...
12 个 terminal factsources 仍由下文固定集合组成；`DH_STAGE_QDR_9_PLAN.md` 与 `DH_STAGE_QDR_9_IMPLEMENTATION_WORK_ORDER.md` 是当前 Stage 的 process authority 与入口，但不增加 terminal block 扫描基数。长期 machine contract 不得依赖这些可裁剪 process documents。
