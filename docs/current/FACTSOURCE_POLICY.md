...
## Terminal current authority — 2026-07-25 Stage-QDR-9 B1 publication accepted
...
Checkstyle: 0
Spotless: PASS
Full regression (this documentation task): NOT_RERUN / REUSED EXACT-SHA CI 30104473274
PostgreSQL/Testcontainers (this documentation task): NOT_RERUN / REUSED EXACT-SHA CI POSTGRESQL 17.10 / ZERO MANDATORY SKIPS
...
Scope invariants: PASS / 3 OF 3 (VALIDATION ⊆ READ / FIXABLE_BLOCKER ⊆ WRITE_ALLOWLIST / CURRENT_FACTSOURCE_SCAN ⊆ WRITE_ALLOWLIST)
CURRENT_FACTSOURCE_CONSISTENCY: PASS / 12 OF 12 / 1 AUTHORITY HASH / 0 CONFLICTS
B2 implementation: ALLOWED / NEXT TASK ONLY
B2 capacity: DEFERRED / KNOWN_LIMITATION
Production capacity: NOT_PROVEN
current task: DH-STAGE-QDR-9-B1-PUBLICATION-AUTHORITY-BLOCKER
current task status: AUTHORITY UPDATE / LOCAL COMMIT AND EXACT-SHA CI PENDING
next action after authority commit publication and CI: DH-STAGE-QDR-9-B2-JDBC-PERSISTENCE-AND-TRANSACTIONAL-IDEMPOTENCY-IMPLEMENTATION
ALLOW_STAGE_QDR_9_B2_IMPLEMENTATION: YES / NEXT TASK ONLY / AUTHORITY COMMIT EXACT-SHA CI REQUIRED
...
## Post-pruning terminal current factsource 集合

以下 12 个路径构成本轮裁剪后的 terminal current factsource 集合：
...
权威层级保持不变：`STATUS.md` 与 `WORK_ORDER.md` 是 primary current authority；`FACTSOURCE_POLICY.md` 是 policy authority；`AGENTS.md`、`CLAUDE.md` 与 `CODEX_PROJECT_INSTRUCTIONS.md` 是执行指导；两个 README 是入口索引；`ROADMAP.md`、`TESTING.md`、`WORKLOG.md` 与 `ARCHIVE_INDEX.md` 提供路线、验证、执行和归档指针。入口与 supporting 文件不得覆盖 primary current authority。

`docs/gates/stage-qdr-7/` 与 `docs/gates/stage-qdr-8/` 只承担 historical evidence；两个 annotated tag 固定删除前的 exact repository snapshot。已删除的 `docs/current/DH_STAGE_QDR_7*.md` 与 `docs/current/DH_STAGE_QDR_8*.md` 不再是 current factsources，不得由历史文档重新提升为当前 authority。
...
本轮事实源同步只记录本地已验证实现与待远端 CI 状态；不把 attribution foundation 解释为在线学习、NQ 集成或 production capacity。
