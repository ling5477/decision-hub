...
- 当前 DH 实际执行路径为 `E:/Project/decision-hub`；不得自动切换到旧环境路径 `F:/project/decision-hub`。
...
- stage-qdr-3 B5 首次 close review 已执行但 `NOT_ACCEPTED`，原因是 root README 与 `docs/current` 事实源漂移；本轮 blocker fix 后允许重新执行 B5 close review。
- stage-qdr-3 acceptance 仍为 `NOT YET ACCEPTED`；stage-qdr-3 final close 仍为 `NOT CLOSED`；stage-qdr-4 `NOT STARTED`。
