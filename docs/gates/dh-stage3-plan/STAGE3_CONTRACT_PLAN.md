...
> Created: 2026-05-26
> Updated: 2026-05-26 (Stage3-B1 IMPLEMENT 落地，仅追加字段描述与 4 份契约测试；
>          contracts/json-schema/* 16 份结构语义不变；contracts/openapi.yaml 端点语义不变)
> Parent: `docs/current/STAGE3_PLAN.md`
> Scope: 规划 Stage3 联调阶段两条链路的契约规则。本文件不修改任何已落地契约语义，仅规划。
>        Stage3-B1 落地时新增 4 份 contract 测试类（NqFeedbackEnvelopeSchemaContractTest /
>        DhBacktestRequestSchemaContractTest / BacktestResultSnapshotSchemaContractTest /
>        OpenApiContractAlignmentTest）作为口径硬约束。
