...
`docs/gates/stage-qdr-5/` 记录 Stage-QDR-5 Model Gateway Observability / Provider Readiness Hardening 的 `CLOSED / ACCEPTED / ARCHIVED / TAGGED` 归档入口，后续 tag 名为 `dh-stage-qdr-5-close`。

Stage archive 必须形成 self-contained archive packet；单个 `README.md` 不满足阶段归档要求。Stage-QDR-4 与 Stage-QDR-5 已补齐 `PLAN.md`、`IMPLEMENTATION_WORK_ORDER.md`、`BATCH_SUMMARY.md`、`VALIDATION_EVIDENCE.md`、`FINAL_CLOSE_REVIEW.md`、`ARCHIVE_CLOSE.md`、`STATUS_SNAPSHOT.md` 等 packet 文件；Stage-QDR-5 tag close 已完成；后续 Stage-QDR-6 只能作为独立 planning-first 任务启动。
