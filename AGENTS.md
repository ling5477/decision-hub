...
Stage-QDR-9 当前仅完成规划。下一任务 `DH-STAGE-QDR-9-IMPLEMENTATION-WORK-ORDER` 必须先枚举 migration、Repository、read model 的 exact files 并达到 scope subset `6 / 6 PASS`；不得直接实现，不得新增 API，不得形成 automatic learning。
