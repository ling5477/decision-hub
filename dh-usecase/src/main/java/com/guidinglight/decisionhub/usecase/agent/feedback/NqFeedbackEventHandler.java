...
public interface NqFeedbackEventHandler {
   ...
   * 处理事件。
   *
   * @param envelope 已通过契约校验的 NQ feedback envelope。
   * @param tenantId 已认证请求上下文解析出的 tenant，禁止 handler 自行使用硬编码默认 tenant。
   */
  void handle(NqFeedbackEnvelope envelope, String tenantId);
}
